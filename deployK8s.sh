...
sudo systemctl disable --now docker >/dev/null 2>&1
mkdir /etc/apt/keyrings >/dev/null 2>&1

# Reset kubeadm if already initialized
...
echo "deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v${k8sVersion:0:4}/deb/ /" | sudo tee /etc/apt/sources.list.d/kubernetes.list
 >/dev/null 2>&1

echo "[TASK 9] Add K8s repository"
if [[ $# -gt 3 ]]; then
        curl -x $4 -fsSL https://pkgs.k8s.io/core:/stable:/v${k8sVersion:0:4}/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg >/dev/null 2>&1
else
        curl -fsSL https://pkgs.k8s.io/core:/stable:/v${k8sVersion:0:4}/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg >/dev/null 2>&1
fi
...
then
	...
	if [[ $# -gt 3 ]]; then
        ...
        kubeadm init --pod-network-cidr 192.168.0.0/16 | tee /root/init.log >/dev/null 2>&1
